...
public class Logger implements UncaughtExceptionHandler
{
	...
	private static UncaughtExceptionHandler previousMainHandler;
	private static UncaughtExceptionHandler previousDefaultHandler;
	...
	public static void initLogger() throws IOException
	{
		...
		previousMainHandler = t.getUncaughtExceptionHandler();
	...
	{
        Thread t = Looper.getMainLooper().getThread();
		t.setUncaughtExceptionHandler(previousMainHandler);
		Thread.setDefaultUncaughtExceptionHandler(previousDefaultHandler);
		previousMainHandler = null;
	...
	{			
        synchronized (_syncObject)
        {
            if(l!=null)
				...
				// Error reporting must remain visible when diagnostic logging is disabled
				// (Logger.initLogger is intentionally skipped in that mode).
	...
	public void uncaughtException(Thread thread, Throwable ex)
	{
		...
		// Delegate to the handler that was installed before EDS. This keeps Android's
		// normal crash handling intact and avoids destroying the thread group directly.
		UncaughtExceptionHandler delegate = thread == Looper.getMainLooper().getThread() ?
				previousMainHandler : previousDefaultHandler;
		if(delegate != null && delegate != this)
		{
			try { delegate.uncaughtException(thread, ex); }
