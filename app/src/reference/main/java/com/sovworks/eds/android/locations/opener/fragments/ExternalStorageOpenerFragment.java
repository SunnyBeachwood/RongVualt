...
public class ExternalStorageOpenerFragment extends LocationOpenerBaseFragment
{
    public static class CheckLocationWritableTaskFragment extends TaskFragment
    {
        ...
        {
            // Permission probing must not create a plaintext marker file on
            // shared storage. A conservative false result simply requests the
            // persisted SAF tree grant below.
            File root = new File(loc.getRootPath());
            return root.isDirectory() && root.canWrite();
        }
        @Override
