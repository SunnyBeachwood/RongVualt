...
class ArchiveFileJob(
    ...
    override fun run() {
        ...
        val channel = archiveFile.openArchiveOutputChannel()
...
/**
 * Android 16/Oplus blocks the hidden NioUtils bridge used by the legacy local
 * provider. A standard FileOutputStream channel remains seekable, which is
 * required when libarchive finalizes ZIP and 7z output.
 */
@Throws(IOException::class)
fun Path.openArchiveOutputChannel(): SeekableByteChannel =
    if (isLinuxPath && !shouldUseRoot(this)) {
        ...
        // releases. Create the document through the provider, then expose its
        // proxy descriptor as a standards-compliant seekable channel using
        // public POSIX APIs only.
        newOutputStream(StandardOpenOption.CREATE_NEW, StandardOpenOption.WRITE).close()
        val descriptor = try {
            DocumentResolver.openParcelFileDescriptor(this as DocumentResolver.Path, "rw")
            ...
            throw IOException("Unable to open archive output descriptor", error)
        }
        ParcelFileDescriptorSeekableByteChannel(
            descriptor,
            readable = true,
            writable = true,
        )
