...
object DocumentResolver {
    ...
    fun copy(
        ...
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N
            && sourcePath.hasSameDocumentTree(targetPath) && !sourcePath.isCopyUnsupported) {
    ...
    private val Path.isCopyUnsupported: Boolean
        get() {
            ...
            // RongVault's unlocked-volume provider deliberately exposes proxy
            // descriptors instead of a provider-side copyDocument() API.  A
            // Binder call to the default DocumentsProvider implementation can
            // surface as a wrapped remote exception rather than the local
            // UnsupportedOperationException handled by copyApi24(), leaving
            // the UI copy action looking inert.  Keep these copies on the
            // cancellable stream path; moves inside the same tree still use
            // the provider's atomic moveDocument() implementation.
            return authority in COPY_UNSUPPORTED_AUTHORITIES
                || authority?.endsWith(".unlocked") == true
        }
    ...
    fun move(
        ...
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N
            && sourcePath.hasSameDocumentTree(targetPath) && !sourcePath.isMoveUnsupported) {
    ...
    /** A provider authority can expose several independent trees.  Only one
     * tree is an atomic filesystem, while cross-tree moves must copy then
     * delete the source after the copy is committed. */
    private fun Path.hasSameDocumentTree(other: Path): Boolean = treeUri == other.treeUri
