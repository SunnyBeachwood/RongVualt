...
class FileProvider : ContentProvider() {
        ...
        loop@ for (column in projectionColumns) {
            ...
            when (column) {
                ...
                OpenableColumns.SIZE -> {
                    val size = try {
                        path.withProviderIdentity { size() }
                    } catch (e: IOException) {
                        e.printStackTrace()
                        null
                    }
                ...
                DocumentsContract.Document.COLUMN_LAST_MODIFIED -> {
                    val lastModified = try {
                        path.withProviderIdentity { getLastModifiedTime().toMillis() }
                    } catch (e: IOException) {
                        e.printStackTrace()
                        null
                    }
    ...
    override fun openFile(uri: Uri, mode: String): ParcelFileDescriptor? {
        ...
        if (path.isDocumentPath) {
            // A DocumentsProvider already owns the descriptor lifecycle and,
            // for unlocked containers, supplies a fully seekable proxy FD.
            // Delegating it also avoids Material Files' reflective
            // NioUtils.newFileChannel() bridge, which no longer exists on
            // current Android releases. Clear the receiving application's
            // Binder identity before the nested provider call: the outer
            // FileProvider URI is the granted capability, while the wrapped
            // private tree URI is intentionally never granted to receivers.
            return try {
                path.withProviderIdentity {
                    DocumentResolver.openParcelFileDescriptor(
                        this as DocumentResolver.Path,
                        mode,
                    )
                }
            } catch (error: Exception) {
                throw FileNotFoundException("Unable to open shared document").apply {
                    initCause(error)
                }
            }
        }
    ...
    /**
     * Calls made by a ContentProvider retain the external caller's Binder
     * identity. Nested access to our private DocumentsProvider must instead
     * run as RongVault after the framework has already validated the grant on
     * this FileProvider URI.
     */
    private inline fun <T> Path.withProviderIdentity(block: Path.() -> T): T {
        if (!isDocumentPath) return block()
...
val Path.fileProviderUri: Uri
    get() {
        ...
        if (isDocumentPath && !isUnlockedVolumeDocumentPath) {
            try {
                return documentUri
            } catch (e: IOException) {
                e.printStackTrace()
...
/** Unlocked-volume provider roots are process-local sessions.  Sharing their
 * raw DocumentsProvider URI makes receiving apps depend on that private
 * authority, so route them through our grantable streaming FileProvider. */
private val Path.isUnlockedVolumeDocumentPath: Boolean
    get() = isDocumentPath && runCatching { documentUri.authority?.endsWith(".unlocked") == true }.getOrDefault(false)
