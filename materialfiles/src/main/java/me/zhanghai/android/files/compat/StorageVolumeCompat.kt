...
val StorageVolume.pathCompat: String
    get() =
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            // getPath() was a hidden API and is absent on Android 16. directory is the
            // public replacement on Android 11+, and is available for mounted volumes.
            requireNotNull(directory) { "Mounted storage volume has no directory: $this" }.path
        } else {
...
val StorageVolume.pathFileCompat: File
    get() =
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            requireNotNull(directory) { "Mounted storage volume has no directory: $this" }
        } else {
            File(pathCompat)
        }
