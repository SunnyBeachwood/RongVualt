...
class FtpServerService : Service() {
    ...
    private fun doStart() {
        ...
        try {
            ...
            require(isShareableHomeDirectory(homeDirectory)) {
                "FTP server cannot share an unlocked volume, remote storage, or an archive"
            }
    ...
    private fun isShareableHomeDirectory(path: java8.nio.file.Path): Boolean {
        if (path.isArchivePath) return false
        return when (path.toUri().scheme) {
            "file" -> true
            "document" -> runCatching { path.documentTreeUri }
                .getOrNull()
                ?.authority
                ?.endsWith(".unlocked")
                ?.not()
