...
import me.zhanghai.android.files.provider.common.ParcelFileDescriptorSeekableByteChannel
...
object DocumentFileSystemProvider : FileSystemProvider(), PathObservableProvider, Searchable {
    ...
    override fun newOutputStream(file: Path, vararg options: OpenOption): OutputStream {
        ...
        if (create || createNew) {
            ...
            if (!exists) {
                ...
                return try {
                    SyncingAutoCloseOutputStream(Resolver.openParcelFileDescriptor(uri, mode))
                } catch (e: ResolverException) {
                    throw e.toFileSystemException(uri.toString())
                }
            }
        ...
        return try {
            SyncingAutoCloseOutputStream(DocumentResolver.openParcelFileDescriptor(file, mode))
        } catch (e: ResolverException) {
            throw e.toFileSystemException(file.toString())
        }
    }
    ...
    /**
     * Proxy descriptors may buffer writes until fsync.  Text saves and small
     * archive outputs often close without an explicit flush from the caller,
     * so make close a durable commit before releasing the provider handle.
     */
    private class SyncingAutoCloseOutputStream(
    ...
    override fun newByteChannel(
        ...
        val optionsSet = options.toMutableSet()
        val create = optionsSet.remove(StandardOpenOption.CREATE)
        val createNew = optionsSet.remove(StandardOpenOption.CREATE_NEW)
        val openOptions = optionsSet.toOpenOptions()
        val mode = openOptions.toDocumentMode()
        ...
        if (create || createNew) {
            ...
            if (!exists) {
                val uri = try {
                    ...
                    throw error.toFileSystemException(file.toString())
                }
                descriptor = try {
                    Resolver.openParcelFileDescriptor(uri, mode)
                } catch (error: ResolverException) {
                    throw error.toFileSystemException(uri.toString())
                }
            }
        }
        if (descriptor == null) {
        ...
        return ParcelFileDescriptorSeekableByteChannel(
            descriptor,
            readable = openOptions.read,
            writable = openOptions.write,
            syncWrites = openOptions.sync || openOptions.dsync,
        ).apply {
            if (openOptions.append) position(size())
        }
    }
