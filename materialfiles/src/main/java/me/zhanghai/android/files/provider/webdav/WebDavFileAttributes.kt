            ...
            when {
                response.isSuccess() -> {
                    val lastModifiedTime = FileTime.from(
                        response.lastModifiedTime?.let {
                            ThreeTenInstant.ofEpochMilli(it.toEpochMilli())
                        } ?: ThreeTenInstant.EPOCH
                    )
                    val lastAccessTime = lastModifiedTime
                    val creationTime =
                        response.creationTime?.let {
                            FileTime.from(ThreeTenInstant.ofEpochMilli(it.toEpochMilli()))
                        } ?: lastModifiedTime
