...
/**
 * Stores a Parcelable preference in the Android Keystore-backed vault.  This
 * is used for the storage list because its parcel can contain network
 * credentials; SharedPreferences retains only a non-sensitive reference.
 */
class SecureParcelValueSettingLiveData<T>(
    @StringRes keyRes: Int,
    private val defaultValue: T
) : SettingLiveData<T>(keyRes, 0) {
    init {
        init()
    }
    ...
    override fun getValue(
        ...
        val encoded = if (stored == reference) {
            CredentialVault.get(reference)
        } else {
            // Existing Material Files snapshots store the parcel directly.
            // Decode first, then rewrite it below without losing entries.
            stored
        } ?: return defaultValue
        val value = runCatching { encoded.asBase64().toParcelValue() }.getOrNull()
            ?: return defaultValue
        if (stored != reference) putValue(sharedPreferences, key, value)
        return value
    }

    override fun putValue(sharedPreferences: SharedPreferences, key: String, value: T) {
        val reference = reference(key)
        CredentialVault.put(reference, value.toParcelBase64().value)
        sharedPreferences.edit { putString(key, reference) }
    }

    private fun reference(key: String) = "parcel-setting:$key"
