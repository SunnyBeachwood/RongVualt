...
private val runtimeRootItems: List<NavigationItem>
    get() = RuntimeNavigationRoots.roots.value.orEmpty().map(::RuntimeRootItem)

private class RuntimeRootItem(private val root: RuntimeNavigationRoot) : PathItem(root.path), NavigationRoot {
    override val id: Long get() = root.id.hashCode().toLong()
    override val iconRes: Int get() = root.iconRes
    override fun getTitle(context: Context): String = root.title
    override fun getSubtitle(context: Context): String? = root.subtitle
    override fun getName(context: Context): String = root.title
}
...
private val menuItems: List<NavigationItem>
    ...
    get() = listOf(
        ...
        // RongVault owns the product About page. The embedded library remains
        // credited in that page's open-source notices instead of presenting
        // the upstream standalone-app identity here.
    )
