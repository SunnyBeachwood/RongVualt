...
object ArchiveReader {
    ...
    private fun openArchive(
        ...
        // Android 16/Oplus blocks the hidden NioUtils API used by the legacy
        // Linux provider. Adapt a standard FileInputStream channel instead;
        ...
        val channel = when {
            ...
            file.isDocumentPath -> {
                ...
                try {
                    // The unlocked-volume DocumentsProvider returns a seekable
                    // proxy descriptor.  AutoCloseInputStream exposes its public
                    // FileChannel without relying on the removed hidden
                    // NioUtils.newFileChannel() API, preserving 7z seek support.
                    val descriptor = DocumentResolver.openParcelFileDescriptor(
                        file as DocumentResolver.Path,
                        "r",
                    )
                    input = ParcelFileDescriptor.AutoCloseInputStream(descriptor)
                    CacheSizeSeekableByteChannel(FileChannels.from(input.channel))
                } catch (e: Exception) {
