...
- 内置文件管理器提供目录浏览、搜索、排序、显示隐藏文件、新建、重命名、删除以及
  卷内复制/移动等操作。
- RongVault 1.3.0 在同一文件管理器中原生集成 ZipXtract：可浏览并提取 ZIP/JAR、7z、
  RAR/RAR5、分卷包、tar.*、Brotli 和 LZ4；ISO/CAB/DEB/RPM 等通用格式及 Lzip 使用
  libarchive 兼容回退。通过简洁/高级选项创建 ZIP、7z、TAR，并在安全可写位置
  添加或删除未加密 7z 条目。外部 `ACTION_VIEW`、`ACTION_SEND` 和
  `ACTION_SEND_MULTIPLE` 也进入同一套流程。本版本状态为**代码完成，未编译/未验证**。
  上游快照和 port map 可通过 `tools/sync-zipxtract-upstream.sh` 刷新；映射源变化会提示人工复核。
